//[1]:상속(Inheritance)이란?
// 말 그대로 부모 클래스가 가지고 있는 속성(변수)들과 동작/기능(메소드)들을 그대로 물려 받아 새로운 클래스를 만드는 것
...
// 상속 받아 새롭게 만든ㄹ어진 클래스는 말 그대로 자식 클래스(Child class) 또는 서브/하위 클래스(Sub class)라고 부른다
...
// 상속 받은 메소드라고 해도 필요에 따라서 자식 클래스에서 용도를 변경해서 사용하는 것도 가능

//[3]:상속의 사용
// 기존 부모 클래스를 확장한다는 개념 --> extends 키워드 사용
// 부모 클래스의 멤버 필드, 메소드는 상속이 가능하나 생성자는 상속이 안된댜
// 부모 클래스의 접근 제한자 private인 경우에는 아무리 자식 클래스가 상속을 받았다 하더라도 접근 불가능

public class Oop_InheritanceSample1 {
