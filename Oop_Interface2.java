...
public class Oop_Interface2 {

	public static void main(String[] args) {
		
		// [1]:인터페이스
		// 추상 클래스와 거의 비슷하나 그 추상화 정도가 더 높다(더 엄격). --> 일반 메소드나 멤버 필드(변수)를 가질 수 없다.
		// 표준화 및 규격을 인터페이스로 제공 --> 일종의 "설계도"개념.
		// 따라서, 어떤 클래스가 해당 인터페이스를 사용(상속)한다면 인터페이스에 선언되어져 있는 메소드를 구현
		// 인터페이스는 interface 키워드를 사용
		// 추상 클래스와 같이 메소드의 구체적인 내용은 기술되어져 있지 않으므로 인터페이스를 상속받은 클래스에서 재정의(오버라이딩)하여 사용
		
		// [2]:상속
		// 클래스는 "단일 상속"만 가능, 인터페이스는 "다중 상속"이 가능 --> 가장 큰 차이점
		// class --> extends,	interface --> implements --> 다중 상속을 구현 --> A,B --> 콤마(,)로 분리
		// 죽, 이를 이용하면 여러 개의 인터페이스로 부터 메소드를 받아올 수 있게 된다. --> 다중 상속 구현.
		
		// [3]: 장점
		// 인터페이스를 이용하면 메소드의 추상적인 "선언"과 그 메소드들을 구체적인 "구현"부분을 분리시킬 수 있다. --> 매우 큰 장점.
